...
const TABS = [
    ...
    { label: "Attendance Log", icon: <CalendarMonth />, idx: 1 },
    { label: "Roster Control", icon: <People />, idx: 2 },
    { label: "Grade Matrix", icon: <Grade />, idx: 3 },
    { label: "Task Nodes", icon: <Layers />, idx: 4 },
    { label: "Analytics Hub", icon: <BarChart />, idx: 5 },
];
...
export default function TeacherDashboard() {
        ...
        <Box sx={{
            ...
            <Box sx={{ p: 3, display: "flex", alignItems: "center", justifyContent: sidebarOpen ? "space-between" : "center", minHeight: 100 }}>
                {sidebarOpen && (
                    <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                        <Box sx={{ width: 44, height: 44, borderRadius: 3, background: THEME_G.cyan, display: "flex", alignItems: "center", justifyContent: "center" }}>
                            <Terminal sx={{ color: "white", fontSize: 24 }} />
                        </Box>
                        <Box>
                            <Typography variant="subtitle1" fontWeight={1000} sx={{ letterSpacing: -0.5 }}>HUB-X</Typography>
                            <Typography variant="caption" sx={{ opacity: 0.5, fontWeight: 800, textTransform: "uppercase", fontSize: "0.6rem" }}>Instructor Node</Typography>
                        </Box>
                    </Box>
                )}
                <IconButton onClick={() => setSidebarOpen(!sidebarOpen)}>{sidebarOpen ? <ChevronLeft /> : <MenuIcon />}</IconButton>
            </Box>
                    ...
                    <ListItem key={item.idx} disablePadding sx={{ mb: 1 }}>
                        <Button
                            ...
                            sx={{
                                ...
                                px: sidebarOpen ? 2.5 : 0, py: 1.8, borderRadius: 4,
                                ...
                                "& .MuiButton-startIcon": { mr: sidebarOpen ? 2 : 0 },
            ...
            <Box sx={{ p: 2 }}>
                <Button fullWidth onClick={toggleColorMode} startIcon={mode === "dark" ? <LightMode /> : <DarkMode />} sx={{ color: "text.secondary", py: 1.5, borderRadius: 4 }}>
                    {sidebarOpen && <Typography variant="body2" fontWeight={700}>SENSORS</Typography>}
                </Button>
    ...
    const OverviewTab = () => (
        <Box>
            <Box sx={{ mb: 6 }}>
                ...
                <Typography variant="subtitle1" color="text.secondary" fontWeight={700} sx={{ letterSpacing: 1.5, opacity: 0.7 }}>INSTRUCTOR: {user?.name?.toUpperCase()} // {user?.department?.toUpperCase()} SECTOR</Typography>
            </Box>

            <Grid container spacing={3} sx={{ mb: 6 }}>
                <Grid item xs={12} sm={6} md={3}>
                    <StatHUD label="Modules" value={courses.length} icon={<Layers />} grad={THEME_G.cyan} color="#06b6d4" suffix="LIVE" />
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                    <StatHUD label="Students" value={students.length} icon={<People />} grad={THEME_G.indigo} color="#6366f1" suffix="+12%" />
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                    <StatHUD label="Assignments" value={assignments.length} icon={<Assignment />} grad={THEME_G.violet} color="#8b5cf6" suffix="QUEUED" />
                </Grid>
                <Grid item xs={12} sm={6} md={3}>
                    <StatHUD label="Avg. Score" value="84%" icon={<AutoAwesome />} grad={THEME_G.emerald} color="#10b981" suffix="▲ 2.5" />
                </Grid>
            </Grid>
            ...
            <Grid container spacing={3}>
                    ...
                    <GlassCard sx={{ p: 4, height: "100%" }}>
                        ...
                        <Box sx={{ height: 350 }}>
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={[
                                    { name: "W1", score: 72 }, { name: "W2", score: 78 },
                                    { name: "W3", score: 75 }, { name: "W4", score: 84 },
                                    { name: "W5", score: 82 }, { name: "W6", score: 88 },
                                ]}>
                                    <defs>
                ...
                <Grid item xs={12} md={4}>
                    <GlassCard sx={{ p: 4 }}>
                        ...
                        <Stack spacing={3}>
                            {recentComms.slice(0, 4).map((ann, i) => (
                                <Box key={ann._id || i} sx={{ display: "flex", gap: 2 }}>
                                    <Avatar sx={{ width: 36, height: 36, background: THEME_G.slate, fontSize: "0.8rem", fontWeight: 900 }}>HUD</Avatar>
                                    <Box>
                                        <Typography variant="subtitle2" fontWeight={1000} noWrap>{ann.title}</Typography>
                                        <Typography variant="caption" color="text.secondary" sx={{ display: "-webkit-box", WebkitLineClamp: 1, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{ann.body}</Typography>
                                    </Box>
                                </Box>
                            ))}
                        </Stack>
                ...
                <TableContainer>
                    <Table>
                        <TableHead sx={{ background: alpha("#06b6d4", 0.05) }}>
                            <TableRow>
                                {["Personnel", "ID Hash", "Telemetry Status"].map(h => (
                        ...
                        <TableBody>
                            {students.map((s, i) => (
                                <TableRow key={s._id}>
                                    <TableCell sx={{ py: 2.5 }}>
                                        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                                            <Avatar sx={{ background: THEME_G.cyan }}>{s.name[0]}</Avatar>
                                            <Typography variant="subtitle2" fontWeight={1000}>{s.name}</Typography>
                                        </Box>
                                    </TableCell>
                                    <TableCell sx={{ fontFamily: "monospace", opacity: 0.6 }}>{s.studentId}</TableCell>
                                    <TableCell>
                                        <FormControlLabel
    ...
    return (
        <Box sx={{ display: "flex", bgcolor: isDark ? "#05060f" : "#f8fafc", minHeight: "100vh", overflow: "hidden" }}>
            <Box sx={{ position: "fixed", top: "-10%", right: "-10%", width: "50%", height: "50%", background: "radial-gradient(circle, rgba(6, 182, 212, 0.08), transparent 70%)", filter: "blur(100px)", zIndex: 0 }} />
                    ...
                    <Box>
                        ...
                        {activeTab === 2 && (
                            <Box sx={{ textAlign: "center", py: 20 }}>
                                <People sx={{ fontSize: 80, opacity: 0.1, mb: 2 }} />
                                <Typography variant="h5" fontWeight={1000}>Personnel Link Pending</Typography>
                                <Typography variant="body2" color="text.secondary">Fetching high-fidelity student records from the core matrix...</Typography>
                            </Box>
                        )}
                        {activeTab === 3 && (
                            <Box sx={{ textAlign: "center", py: 20 }}>
                                <Grade sx={{ fontSize: 80, opacity: 0.1, mb: 2 }} />
                                <Typography variant="h5" fontWeight={1000}>Grade Vectoring Offline</Typography>
                                <Typography variant="body2" color="text.secondary">Awaiting authorization from the academic registrar node.</Typography>
                            </Box>
                        )}
                        {activeTab === 4 && (
                            <Box>
                                <Box sx={{ mb: 4, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                    <Typography variant="h4" fontWeight={1000}>Task Nodes</Typography>
                                    <Button variant="contained" startIcon={<Add />} onClick={() => setAssignDialogOpen(true)} sx={{ background: THEME_G.violet, borderRadius: 3, fontWeight: 900 }}>Deploy Task</Button>
                                </Box>
                                <Grid container spacing={3}>
                                    {assignments.map((a, i) => (
                                        <Grid item xs={12} md={6} key={a._id || i}>
                                            <GlassCard sx={{ p: 4 }}>
                                                <Box sx={{ display: "flex", justifyContent: "space-between", mb: 2 }}>
                                                    ...
                                                    <Chip label={a.status} size="small" sx={{ fontWeight: 900, bgcolor: alpha("#8b5cf6", 0.1), color: "#8b5cf6" }} />
                                                </Box>
                                                <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>{a.description}</Typography>
                                                <Divider sx={{ mb: 2, opacity: 0.05 }} />
                                                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                                    <Typography variant="caption" fontWeight={900} sx={{ opacity: 0.5 }}>DEADLINE: {new Date(a.deadline).toLocaleDateString()}</Typography>
                                                    <Button size="small" sx={{ color: "primary.main", fontWeight: 900 }}>RECAP</Button>
                                                </Box>
                                            </GlassCard>
                                        </Grid>
                                    ))}
            ...
            <Dialog
                ...
                PaperProps={{ sx: { background: isDark ? "#0f172a" : "white", borderRadius: 8, p: 2, maxWidth: 500, width: "100%" } }}
            >
                <DialogTitle sx={{ fontWeight: 1000, fontSize: "1.5rem", textAlign: "center" }}>Deploy Task Node</DialogTitle>
                <DialogContent>
                    <Stack spacing={3} sx={{ mt: 2 }}>
                        <TextField label="Node Title" fullWidth value={newAssignment.title} onChange={e => setNewAssignment({ ...newAssignment, title: e.target.value })} />
                        <TextField label="Subroutine Description" multiline rows={3} fullWidth value={newAssignment.description} onChange={e => setNewAssignment({ ...newAssignment, description: e.target.value })} />
                        <TextField label="Termination Date" type="date" fullWidth InputLabelProps={{ shrink: true }} value={newAssignment.deadline} onChange={e => setNewAssignment({ ...newAssignment, deadline: e.target.value })} />
                    </Stack>
                </DialogContent>
                <DialogActions sx={{ p: 3 }}>
                    <Button onClick={() => setAssignDialogOpen(false)} sx={{ fontWeight: 900 }}>Abort</Button>
                    <Button variant="contained" onClick={handleDeployAssignment} sx={{ background: THEME_G.violet, borderRadius: 3, fontWeight: 1000 }}>Initiate</Button>
                </DialogActions>
            </Dialog>
        </Box>
    );
